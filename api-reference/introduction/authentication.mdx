...
> How to authenticate with the Bleepay API — bearer tokens and API keys.
...
| `RegisteredUserOrApiKeyGuard` | JWT with `userId`/`deviceId` or API key | Finance, widget, voucher, and session operations |
