---
title: "Get Deposit Event"
icon: "angle-right"
---

> Get a single deposit event by ID.

`GET /api/v1/widgets/deposit-events/:eventId`

Retrieves a specific deposit event by its unique identifier. The event must belong to a deposit session owned by the authenticated business.
...
|------|------|----------|-------------|
| `eventId` | `string` | Yes | The deposit event ID |

## Request example

```shell
curl --request GET 'https://payments.bleepay.com/api/v1/widgets/deposit-events/evt_abc123' \
...
| `404` | `not_found` | Deposit event not found |
