---
title: "Query Deposit Events"
icon: "angle-right"
---

> List deposit events (state transitions) for a business.

`GET /api/v1/widgets/deposit-events`

Returns a paginated list of deposit events that record every state transition in a deposit session's lifecycle. Each event has a type indicating what changed, and a payload with the full event data.
...
```shell
curl --request GET 'https://payments.bleepay.com/api/v1/widgets/deposit-events?businessId=biz_abc123&type=deposit.confirmed&since=1717200000000&limit=20' \
