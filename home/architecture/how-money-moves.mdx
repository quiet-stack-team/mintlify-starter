...
**C. Crypto in, fiat out (off-ramp — in development).** Payer sends crypto → (optional swap) → **liquidation address held by the fiat partner** → partner converts crypto→fiat → fiat to the payee's bank. The fiat partner holds and moves funds across the fiat leg; Bleepay arranges and orchestrates.
...
| Fiat off-ramp (in development) | Fiat partner's liquidation address (transiently) |
