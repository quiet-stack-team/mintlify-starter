...
<ThemeImage img_name="Widget-deposit-same-token.png" alt="Widget deposit (same token)" />
...
<ThemeImage img_name="Voucher-payment-6-digit-code.png" alt="Voucher payment (6-digit code)" />
...
<ThemeImage img_name="FX-crypto-crypto.png" alt="FX crypto-crypto" />
...
<Info>
  Fiat settlement is **in development** via regulated partners and not yet live for merchants; this section describes the designed flow. Crypto and stablecoin settlement are available today.
</Info>
...
2. **A liquidation address is created** — Bleepay contacts a regulated fiat off-ramp partner (one of several), which returns a dedicated crypto deposit address configured to convert incoming crypto to fiat and pay the bank account.
...
<ThemeImage img_name="fx_offramp.png" alt="FX Offramp" />
