...
Always check the `type` field and handle idempotency (e.g. by `orderId` or the transaction hash). Note: the transaction hash field is named `txHash` in webhook payloads and `txId` on the session API — match on the value, not the field name.
