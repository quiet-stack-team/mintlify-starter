...
<Info>
  This guide walks through the payee (merchant/dApp) side of a Bleepay voucher payment. The payer uses a Bleepay-compatible wallet — you don't implement that side.Prerequisites
</Info>
...
<ThemeImage img_name="Flow-overview.png" alt="Flow overview" />

Your responsibility is the **payee side** (right column). The payer side happens in their wallet.

<Steps>
  <Step title="1. Payer opens a context in their wallet">
  ...
  <Step title="Authenticate as the payee">
  ...
  <Step title="Payer signs and resolves">
