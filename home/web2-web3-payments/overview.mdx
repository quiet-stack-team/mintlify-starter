...
2. **Payee reserves a voucher** — The payee (merchant) authenticates using their API key (`x-api-key` header) and reserves a voucher using the context code, receiving a 6-digit code. This creates a secure, short-lived link between the payer's assets and the payment intent.
